    ...
    <section class="content">
        <div class="container-fluid  d-flex   ">
            <div class="col-6 bg-light  shadow rounded py-5">
                ...
                <?php endif; ?>
                <form action="<?php echo site_url('admin/InvoiceViews/add_invoice'); ?>" method="post"
                      enctype="multipart/form-data" id="invoiceForm">
                    <?php echo form_open(); ?>
                    <!-- Hidden Input-->
                    <input type="hidden" name="services" id="hidden_services">
                    <input type="hidden" name="total_price" id="hidden_total_price">
                    <input type="hidden" name="new_invoice_number" value="<?= $new_invoice_number ?>">
                    <input type="hidden" name="clinic_name" value="<?= $default_values->clinic_name ?>">
                    ...
                    <div class="mb-4" dir="rtl">
                        ...
                        <input type="text" class="form-control shadow-sm" value="<?= set_value('patient_name');  ?>" name="patient_name" id="patient_name"
                               placeholder="أدخل اسم المريض">
                    </div>

                    <div class="mb-4" dir="rtl">
                        <label for="tel" class="form-label">رقم الهاتف</label>
                        <input type="text" class="form-control shadow-sm" name="phone_number" value="<?= set_value('phone_number');  ?>" id="phone_number"
            ...
            <div class="col-5 rounded border shadow px-3 " id="invoice-section">
                <div class="col-12 py-3 d-flex justify-content-center overflow-hidden border-bottom ">
                    <div class="rounded-circle bg-dark text-white d-flex align-items-center justify-content-center"
                        ...
                        <?php if ($default_values->logo): ?>
                            <img src="<?php echo base_url($default_values->logo); ?>" alt="الشعار"
                ...
                <div class="col-12 d-flex flex-wrap flex-row-reverse py-3 ">
                    ...
                    <div class="col-md-4 d-flex flex-column align-items-center text-center">
                        ...
                        <div id="phone_number_display"  class="px-3 py-1 fw-bold"><?php if (isset($phone_number)) {
                                echo $phone_number;
                            } ?></div>
                    </div>
                    ...
                    <div class="col-md-4 d-flex flex-column align-items-center text-center ">
                        ...
                        <p id="todayDate" class="px-3 py-1 fw-bold mb-0"></p>
                    </div>
                ...
                <div class="col-12 d-flex flex-wrap flex-row-reverse py-3 ">
                    ...
                    <div class="col-12 d-flex flex-column align-items-center text-center">
                        <div id="patient_name_display" class=" py-1 fw-bold"><?php if (isset($patient_name)) {
                                echo $patient_name;
                            } ?></div>
                    </div>
                ...
                <div class="col-12 py-4">
                    <table class="table table-borderless" id="servicesTable">
                        ...
                        <?php if (!empty($services)) : ?>
                            <?php foreach ($services as $service) : ?>
                                <tr>
                                    <td style="width: 15%;"><?= htmlspecialchars($service['total']); ?></td>
                                    <td style="width: 10%;"><?= htmlspecialchars($service['quantity']); ?></td>
                                    <td style="width: 15%;"><?= htmlspecialchars($service['price']); ?></td>
                                    <td class="doctor-name-display" style="width: 35%;"><?php echo htmlspecialchars($service['name_doctor']); ?></td>
                                    <td style="width: 25%;" data-id="<?= htmlspecialchars($service['id'] ?? ''); ?>">
                    ...
                    <div class="d-flex col-12 mt-1 border-top py-4 total-container">
                        <div class="col-6 text-start" id="totalPrice"><?= htmlspecialchars($grand_total); ?></div>
                            ...
                            <div class="col-md-4 col-12 text-center">
                                <div class="qr-code-box" id="qrCode1Preview">
                                    <?php if (!empty($default_values->image1)): ?>
                                        <img src="<?php echo base_url($default_values->image1); ?>" alt="QR Code 1">
                                    <?php else: ?>
                            ...
                            <div class="col-md-4 col-12 text-center">
                                <div class="qr-code-box" id="qrCode2Preview">
                                    <?php if (!empty($default_values->image2)): ?>
                                        <img src="<?php echo base_url($default_values->image2); ?>" alt="QR Code 2">
                                    <?php else: ?>
                                        <p class="text-muted">لا توجد صورة</p>
                                    <?php endif; ?>
                                </div>
                            ...
                            <div class="col-md-4 col-12 text-center">
                                <div class="qr-code-box" id="qrCode3Preview">
                                    <?php if (!empty($default_values->image3)): ?>
                                        <img src="<?php echo base_url($default_values->image3); ?>" alt="QR Code 3">
                                    <?php else: ?>
                                        <p class="text-muted">لا توجد صورة</p>
                                    <?php endif; ?>
                                </div>
...
<script>
    function getTodayDate() {
        var today = new Date(); // الحصول على التاريخ الحالي
        var day = today.getDate(); // اليوم من الشهر
        var month = today.getMonth() + 1; // الشهر (يبدأ من 0 لذا نضيف 1)
        var year = today.getFullYear(); // السنة

        // إذا كانت اليوم أو الشهر أقل من 10، أضف صفرًا
        day = (day < 10) ? '0' + day : day;
        month = (month < 10) ? '0' + month : month;

        // إرجاع التاريخ بالتنسيق "اليوم/الشهر/السنة"
...
<script>
    function updateHiddenServices() {
        ...
        document.querySelectorAll("#servicesTable tbody tr").forEach(row => {
            ...
            if (columns.length > 0) {
                let serviceColumn = columns[4]; // عمود الخدمة
                let serviceId = serviceColumn.getAttribute("data-id") || null; // جلب ID الخدمة

                services.push({
                    id: serviceId, // إضافة الـ ID
                    total: columns[0].textContent.trim(),
                    quantity: columns[1].textContent.trim(),
                    price: columns[2].textContent.trim(),
                    name_doctor: columns[3].textContent.trim(),
                    service: serviceColumn.textContent.trim()
                });
            }
        });

        // تحديث قيمة الـ input المخفي للخدمات
        document.getElementById("hidden_services").value = JSON.stringify(services);

        // تحديث قيمة الـ input المخفي للإجمالي
        let totalPriceElement = document.getElementById("totalPrice");
        if (totalPriceElement) {
            document.getElementById("hidden_total_price").value = totalPriceElement.textContent.trim();
        }
    }

    // استدعاء الدالة عند إرسال الفورم
