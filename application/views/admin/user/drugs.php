    ...
    <div class="box add_area <?php if(isset($page_title) && $page_title == "Edit"){echo "d-block";}else{echo "hide";} ?>">
      <div class="box-header with-border">
        ...
        <div class="box-tools pull-right">
          <?php if (isset($page_title) && $page_title == "Edit"): ?>
            <a href="<?php echo base_url('admin/drugs') ?>" class="pull-right btn btn-light-secondary btn-sm mt-15"><i class="fa fa-angle-left"></i> <?php echo trans('back') ?></a>
          <?php else: ?>
      ...
      <div class="box-body">
        <form id="cat-form" method="post" enctype="multipart/form-data" class="validate-form" action="<?php echo base_url('admin/drugs/add')?>" role="form" novalidate>
          ...
          <div class="form-group">
            ...
            <input type="text" class="form-control" name="generic_name" value="<?php echo html_escape($drug[0]['generic_name']); ?>" >
          </div>

          <div class="form-group">
            <label>Brand Name </label>
            <input type="text" class="form-control" name="brand_name" value="<?php echo html_escape($drug[0]['brand_name']); ?>" >
          </div>
                    ...
                    <tr id="row_<?php echo html_escape($drug->id); ?>">
                        ...
                        <td>
                          <?php if(!empty($drug->generic_name)): ?>
                            <p class="mb-0 mt-0"><b>Generic name</b> : <?php echo html_escape($drug->generic_name); ?></p>
                          <?php endif; ?>

                          <?php if(!empty($drug->brand_name)): ?>
                            <p class="mb-0 mt-0"><b>Brand name</b> : <?php echo html_escape($drug->brand_name); ?></p>
                          <?php endif; ?>
                          
                          <?php echo character_limiter($drug->details, 80); ?> 
                        </td>
                        
                        <td class="actions" width="12%">
                          <a href="<?php echo base_url('admin/drugs/edit/'.html_escape($drug->id));?>" class="on-default edit-row" data-placement="top" title="<?php echo trans('edit') ?>"><i class="fa fa-pencil"></i></a> &nbsp; 

                          <a data-val="Category" data-id="<?php echo html_escape($drug->id); ?>" href="<?php echo base_url('admin/drugs/delete/'.html_escape($drug->id));?>" class="on-default remove-row delete_item" data-toggle="tooltip" data-placement="top" title="<?php echo trans('delete') ?>"><i class="fa fa-trash-o"></i></a> &nbsp;
                        </td>
                    </tr>
